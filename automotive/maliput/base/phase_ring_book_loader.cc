...
RightOfWayRule::State GetDefaultState(const std::unordered_map<RightOfWayRule::State::Id, RightOfWayRule::State>& states) {
  ...
  DRAKE_UNREACHABLE();  // This would imply that the rule has no states, which should never happen.
}

// Given a set of rules, determine default states for each rule and return them in a new RuleStates object.
// See GetDefaultState() for details on how the default state is determined.
RuleStates CreateDefaultRuleStates(const std::unordered_map<RightOfWayRule::Id, RightOfWayRule> rules) {
  RuleStates result;
  for (const auto& rule : rules) {
    result.emplace(std::make_pair(rule.first, GetDefaultState(rule.second.states()).id()));
  }
  return result;
}

RightOfWayPhaseRing BuildPhaseRing(const RoadRulebook* rulebook, const YAML::Node& phase_ring_node) {
  DRAKE_DEMAND(phase_ring_node.IsMap());
  const RightOfWayPhaseRing::Id ring_id(phase_ring_node["ID"].as<std::string>());
...
std::unique_ptr<api::rules::RightOfWayPhaseBook> BuildFrom(const RoadRulebook* rulebook, const YAML::Node& root_node) {

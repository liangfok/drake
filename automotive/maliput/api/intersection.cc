...
Intersection::Intersection(const Id& id, const std::vector<LaneSRange>& region,
    ...
    : id_(id), region_(region), ring_(ring), phase_provider_(phase_provider) {
  DRAKE_THROW_UNLESS(ring != nullptr);
  DRAKE_THROW_UNLESS(phase_provider != nullptr);
}

const optional<RightOfWayPhaseProvider::Result> Intersection::Phase() const {

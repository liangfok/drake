#include "drake/systems/plants/model.h"
...
void Model::AddRigidBody(std::unique_ptr<RigidBody> rigid_body) {
  rigid_bodies_.push_back(std::move(rigid_body));
}
...
RigidBody& Model::GetRigidBody(const std::string& name) const {
...
std::vector<RigidBody*> Model::GetRigidBodies() const {
  std::vector<RigidBody*> result;
  for (const auto& map_entry : rigid_bodies_) {

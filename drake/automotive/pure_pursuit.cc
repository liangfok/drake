...
T PurePursuit<T>::Evaluate(const PurePursuitParams<T>& pp_params,
  ...
  const GeoPosition goal_position =
      ComputeGoalPoint(pp_params.s_lookahead(), lane_direction, road, pose);
...
const GeoPosition PurePursuit<T>::ComputeGoalPoint(
    ...
    const RoadGeometry& road, const PoseVector<T>& pose) {
  const maliput::api::Lane* const lane = lane_direction.lane;
  ...
  const LanePosition position =
      lane->ToLanePosition({pose.get_isometry().translation().x(),
              ...
              pose.get_isometry().translation().z()}, nullptr, nullptr);
  const T s_new =
  ...
  // lanes.
  return lane->ToGeoPosition(LanePosition(s_goal, 0., position.h));
}

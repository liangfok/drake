  ...
  static void VerifyAllShapes(const sensors::Image<uint8_t>& color_image,
    ...
    for (int v = 0; v < color_image.height(); v += 10) {
      ...
      for (int u = 0; u < color_image.width(); u += 10) {
        ...
        ASSERT_NEAR(color_image.at(u, v)[0], *(color + 2), 1.);  // B
        ASSERT_NEAR(color_image.at(u, v)[1], *(color + 1), 1.);  // G
        ASSERT_NEAR(color_image.at(u, v)[2], *(color + 0), 1.);  // R
        ASSERT_NEAR(color_image.at(u, v)[3], *(color + 3), 1.);  // A

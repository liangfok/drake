...
GTEST_TEST(TestGlobalCollisionSensor, BasicCollisionTest) {
  ...
  /* The indices of the position states within `x` are as follows:
    ...
    will result in a separation of 0.11 - 0.1 = 0.01 m, which should be
    sufficient to avoid collision between the two boxes.
  */
  x(0) = 0;
  x(7) = 0.11;
...
GTEST_TEST(TestGlobalCollisionSensor, AttachGlobalCollisionSensorTest) {
  ...
  // TODO(liang.fok) Add a test that ensures the state of the RigidBodyPlant is
  // correctly considered by the output of the global collision sensor.
}

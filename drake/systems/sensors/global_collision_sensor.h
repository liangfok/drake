...
///
/// <B><I>
/// IMPORTANT: This class includes some extreme hacking using
/// RigidBodyPlant, which is scheduled for removal in the near future. Once
/// GeometryWorld exists, this class should be updated to make use of it. In
/// addition, the input of this class should be PoseBundle objects.
/// </I></B>
///
...
/// accessor:
///
///  - get_output_port(): Contains the sensed collision data. It's an abstract
///    port containing a `ContactResults<double>`.
///
...
class GlobalCollisionSensor : public systems::LeafSystem<double> {
 public:
  ...
  /// A constructor that initializes a GlobalCollisionSensor.
  ///
  /// This constructor takes ownership of a RigidBodyTree as opposed to a
  /// `RigidBodyPlant` because it is designed for use in systems that do not
  /// have a `RigidBodyPlant`. Examples include those involving only
  /// non-physics-based vehicles.
  ///
  ...
  ///
  /// This factory method takes ownership of a RigidBodyTree and a descriptor of
  /// an output port that provides the generalized state information to rather
  /// than a RigidBodyPlant because it's designed to work in systems that do not
  /// have a RigidBodyPlant.
  ///
